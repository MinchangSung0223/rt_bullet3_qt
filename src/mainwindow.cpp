    ...
    qsrand(QTime::currentTime().msecsSinceStartOfDay());
      ...
      static double lastPointKey = 0;
        ...
        for(int j =0;j<JOINTNUM;j++){
            switch(display_flag){
                case JOINT_POS: ui->widget_RealTimeGraph->graph(j)->addData(key, robot_info.act.q[j]);break;
                case JOINT_VEL: ui->widget_RealTimeGraph->graph(j)->addData(key, robot_info.act.q_dot[j]);break;
                case JOINT_TORQ: ui->widget_RealTimeGraph->graph(j)->addData(key, robot_info.act.tau[j]);break;
                case EXT_WRENCH: if(j<6){ui->widget_RealTimeGraph->graph(j)->addData(key, robot_info.act.F_ext[j]);break;}
            }
        }
...
void MainWindow::on_pushButton_ExtWrench_clicked()
{
    ...
    //std::lock_guard<std::mutex> lock(g_pages_mutex); 
    robot_info.act.F_ext[0] = dis(gen);
    robot_info.act.F_ext[1] = dis(gen);
    robot_info.act.F_ext[2] = dis(gen);
    robot_info.act.F_ext[3] = dis(gen);
    robot_info.act.F_ext[4] = dis(gen);
    robot_info.act.F_ext[5] = dis(gen);    
}
